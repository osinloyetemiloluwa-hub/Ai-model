...
| 38 | Windows 11: an ACS delegation run (chat auto-delegation / `acs_delegate`) fails immediately with `OSError: [WinError 123] The filename, directory name, or volume label syntax is incorrect: '…\sessions\web:<sid>\acs\runs\…\workers'` | `acs_runtime._run_dir()` built the session component as the RAW chat_key `f"{bridge}:{chat}"` (e.g. `web:<sid>`). The `:` is legal on POSIX but ILLEGAL in a Windows path component (drive/ADS separator) → `os.mkdir` raised WinError 123 and every ACS run died on Windows. The main web-session workdir already sanitises this via `chat_runtime._workdir` → `forge.paths.safe_session_subdir` (the 0.9.49 fix, troubleshooting-adjacent to the ':' session-path class), but the ACS run-dir builder had reinvented the path and bypassed it — a reader≠writer + Windows-illegal-path bug. | **Fixed (2026-07-13):** `_run_dir` routes the session component through the SAME SSOT (`safe_session_subdir(sessions_base, f"{bridge}:{chat}")`) → the `:` is sanitised to `web_<sid>` on Windows AND the ACS run dir lands UNDER the real session dir (POSIX byte-identical, honours any legacy dir). PLUS robustness fallback (user request): if the ACS run still can't be set up — runtime import failure, an un-creatable run dir (a `sess.workdir/acs/runs` pre-flight `mkdir`), or an empty task — the turn now FALLS BACK to the normal Claude Code OS-turn (which does its own Task-tool delegation) via the same `_quota_fallback` path the ACS-quota-exhausted case already used, instead of surfacing the raw error. Tests: `operator/bridges/shared/test_acs_runtime_windows_path.py` (3 cases) + existing `test_acs_quota_fallback.py`/`test_web_delegation.py` (20) still green. |
