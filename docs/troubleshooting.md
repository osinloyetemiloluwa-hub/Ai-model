...
| 34 | Fresh-install/console "Welcome" spoken greeting comes out in English even though `profile.display_language` is set to a non-de/en locale (e.g. `zh`) — not translated, not left in the user's actual language either | `operator/voice/i18n/` ships only `de.json` and `en.json` — no bundle exists for any other locale. `_welcome_check_lang()` (`core/console/corvin_console/routes/setup.py`) resolves the greeting language straight from `profile.display_language` via `i18n.resolve()`, which happily returns e.g. `zh-Hans` — but `i18n.t()`'s own lookup chain (exact locale → base locale → English → literal key, see `i18n.py::t`) then silently falls all the way through to English for every `welcome.*` key, with no warning logged anywhere. Unlike the voice-summary bug (#33 above), there's no per-turn text to run a de/en detector against here — the greeting is assembled fresh from templates, not translating an existing reply — so the `_resolve_voice_output_language` escape hatch doesn't apply to this code path at all. Confirmed live-affecting: this repo's own current `profile.json` has `display_language: "zh"`. | **Not fixed — flagged, not silently patched.** This is a data/preference question (should `display_language` actually be `"de"`?) as much as a code gap, so it wasn't changed unilaterally. Pinned by new tests, `core/console/tests/test_setup_welcome_check.py::TestWelcomeCheckLanguageResolution` (2026-07-12) — calls `_welcome_check_lang()`/`_build_welcome_greeting()` unmocked for `de`/`en`/`zh` profiles and asserts the CURRENT (English-fallback) behavior, so a future bundle addition or fallback change shows up as a deliberate diff here, not a silent flip. If you hit this: either correct `profile.display_language` to `de`/`en` (`corvin-voice profile set display_language de`), or add a real `operator/voice/i18n/zh-Hans.json` bundle with `welcome.*` keys translated. |
