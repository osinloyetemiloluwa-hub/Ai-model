...
run "Node: sticky-progress guard (shared, all channels)" node shared/js/test_sticky_progress.js >/dev/null || fails=$((fails+1))
run "Node: discord sticky-progress wiring"  node discord/test_sticky_progress_wiring.js >/dev/null  || fails=$((fails+1))
run "Node: telegram sticky-progress wiring" node telegram/test_sticky_progress_wiring.js >/dev/null || fails=$((fails+1))
run "Node: slack sticky-progress wiring"    node slack/test_sticky_progress_wiring.js >/dev/null    || fails=$((fails+1))
run "Node: WA sticky-progress wiring"       node whatsapp/test_sticky_progress_wiring.js >/dev/null || fails=$((fails+1))
