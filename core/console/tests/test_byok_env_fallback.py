"""WA-20: /byok/secrets must reflect a key configured the way this codebase's
own docs tell users to configure it (an env var, or a line in
~/.config/corvin-voice/{.env,service.env}) — not only the opt-in BYOK vault
that nothing writes to by default.
"""
